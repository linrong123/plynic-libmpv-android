#!/bin/bash
# build.sh [abi]: the harness (librotprobe.so, classes.dex) into out/<abi>/,
...
"$cc" -shared -fPIC -O1 -Wall -I"$bs/deps/mpv/include" -o "$out/librotprobe.so" \
  "$here/rotprobe.c" -L"$bs/prefix/$abi/lib" -lmpv
javac -source 8 -target 8 -nowarn -cp "$sdk/platforms/android-34/android.jar" \
  -d "$out/classes" "$here/RotateProbe.java" 2>&1 | grep -v -e warning -e '^Note' || true
"$sdk/build-tools/35.0.0/d8" --min-api 29 --lib "$sdk/platforms/android-34/android.jar" \
