...
//
//   host:    fallprobe <seconds> <file> [opt=value ...]   (plynic-libmpv-darwin tools/fallback/run.sh)
//   Android: RotateProbe <libmpv.so> <libfallprobe.so> <WxH>[:private] <file> <seconds>|opt=value ...
//            (RotateProbe.java of tools/rotate-check; "@0" = its Surface; tools/fallback-check/run.sh)
